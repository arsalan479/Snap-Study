...
import {
  ...
  sumbitquizdata,
  qicompdatadelete,
} from "../../Controllers/RoomCreateController/aiquizcompetion.controller.js";
import {
  bookmark,
  bookmarkdelete,
  compdatafetch,
  ...
  fetchbookmark,
  userpost,
} from "../../Controllers/RoomCreateController/bookmark.controller.js";
...
route.post(
  ...
  body("topicName")
    ...
    .matches(/^[A-Za-z\s]+$/)
    .withMessage("Topic Name only taking a alphabates not numbers")
    .isLength({ min: 10 })
    .withMessage("topic name minium define is 10 character")
    .isLength({ max: 100 })
    .withMessage("topic define only 100 maximum character"),
  body("numberofquestions")
    .isString()
    .notEmpty()
    .withMessage("Number of Question is required"),
...
route.post(
  "/competionpost/:compId",
  body("message")
    .notEmpty()
    .isLength({ min: 20 })
    .withMessage("caption must be at least 20 characters long").
  isLength({ max: 2000 }).withMessage(
    "caption must be at maximum 400 character"
  ),
  competionpost
);
