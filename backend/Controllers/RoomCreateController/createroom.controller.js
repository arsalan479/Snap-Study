...
import { getIO } from "../../Utils/sockeio.js";
...
export const getonlinefriends = async (req, res) => {
  try {
    ...
    const user = await UserOne.findById({_id:userId}).populate(
      "friends",
      "displayName status"
    );
...
try {
    ...
    io.to(freindId).emit('quiz-request',{from:{_id:userId},name:req.user.displayName});
    return res.status(200).json({
