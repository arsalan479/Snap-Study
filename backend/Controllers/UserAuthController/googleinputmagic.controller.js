...
import {
  ...
  welcomeVerification,
} from "../../Utils/sendverificationcode.mailer.js";
...
export const googleverifyemailinput = async (req, res) => {
  try {
    ...
    const googleinputuser = await UserOne.findOne({
      ...
      "authMethods.googleuserbyemail.verificationCode email displayName"
    );
    ...
    await welcomeVerification(
      googleinputuser.displayName,
      googleinputuser.email
    );
    ...
    jwtgooglemail(
      googleinputuser,
      res,
      "email verified and genereated jwt token"
    );

    await setonline(googleinputuser._id)
...
export const googleinputlogin = async (req, res) => {
  try {
    ...
    const userlogin = await UserOne.findOne({ email }).select(
      "authMethods.googleuserbyemail.password authMethods.googleuserbyemail.verified role displayName "
    );
...
export const forgetPasswordById = async (req, res) => {
  try {
    ...
    const user = await UserOne.findOne({ email }).select(
    ...
    user.authMethods.googleuserbyemail.resetTokenExpiry =
      Date.now() + 15 * 60 * 1000; //5 min expiry
    await user.save();

    const resetLink = `${process.env.RESET_PASSWORD_LINK}/resetpassword/${token}`;
    const passwordsendemail = await passwordresetverification(
      email,
      resetLink,
      user.displayName
    );
